...
- 新增根目录 `logo.png`（512×512）及可编辑的 `logo.svg`，AstrBot Dashboard 可直接识别插件图标。
- 新增 `/plotclearcache`（别名 `/plotcacheclear`、`/绘图缓存清理`），只清理绘图 PNG，不影响公式和解题图卡。
- 新增绘图缓存文件数/字节数上限、表达式长度/运算量/幂指数/坐标跨度限制，并在配置中开放调节。

### Changed

- `plotting` 与 `geometry` 的 Matplotlib/SymPy 依赖改为按需加载；普通公式渲染不会在插件启动时初始化绘图栈。
- 绘图操作移到后台线程并串行化 Matplotlib 全局状态，避免阻塞 AstrBot 事件循环；渲染失败时会主动回收 figure 和残留文件。
- 绘图缓存键加入渲染配置与采样密度指纹，修改 DPI、配色、视角或网格密度后会自动生成新图。
- 解答卡命中缓存时跳过几何图解析与绘制；默认关闭 HTML renderer 预热以降低启动内存峰值。
- 手动命令和 LLM 绘图工具兼容 `y=...`、`z=...`、`r=...` 及带标签的参数方程写法，并支持逗号、分号和换行分隔表达式。
- HTML 渲染的 Chromium 截图改为进程内串行执行，多人同时触发渲染时不再互相抢占浏览器上下文。
- 临时文件清理加入最小间隔节流，高频消息场景下不再每次渲染都全量扫描缓存目录。

### Fixed

- 修复绘图表达式可以携带未知变量/函数、过大采样配置或极端尺寸导致资源失控的问题。
- 修复绘图缓存中的空文件可能被误判为有效图片的问题。
- 修复形如 `sqrt(-1)`、含虚数单位的表达式导致 `ComplexInfinity` 内部报错的问题，现在会返回明确的“表达式取值非实数”提示。
- 公式图卡与几何图解缓存键补上渲染配置指纹，改字号、主题或图卡宽度后不会再复用旧图。

## [v0.4.13] - 2026-07-05

### Fixed

- 修复 LLM 工具直发图片在 aiocqhttp 返回 `retcode=1200` 超时时可能重复发送的问题：这类超时通常发生在平台已经发起发送后，插件现在会记录 warning 并停止 `event.send` 二次兜底，避免同一张解题图卡发两遍。
- 修复 `Context.send_message` 正常返回 `None` 时被误判为失败的问题；现在只有明确返回 `False` 才会进入下一条发送兜底链路。
...
- 强化 `plot_3d_parametric` 的渲染效果：三维参数曲线现在会按参数 `t` 渐变上色，并显示 `t` 色条，避免正确选中工具后仍只得到单色蓝线图。
- 为三维参数曲线缓存 key 加入渲染版本标记，升级后同一表达式会重新生成新样式图片，不会继续复用旧缓存。

## [v0.4.10] - 2026-07-05

### Fixed

- 强化三维绘图工具选择规则：`plot_3d_function` 只用于 `z=f(x,y)` 曲面，`x=...`、`y=...`、`z=...` 这类关于 `t` 的三维参数方程会被识别为 `plot_3d_parametric`。同时为 `plot_3d_function` 增加兜底转发，避免模型把三维参数曲线误当三维曲面时直接画错图。
...
- 清理 README 中偏内部讨论口吻的配置取舍说明，让 README 更适合作为面向普通使用者的插件说明文档；图片发送压缩参数继续作为 `图片发送` 模块的高级配置项保留。
